...
SoftwareServo myservo2;  // creates name “my servo” as object to control a servo 
...
{ 
  myservo.attach(0); 
  myservo2.attach(1); // attaches the servo on pin 0 to the servo object 
...
void loop() 
{
    for(int i=0; i<5; i++){
        ...
        delay(delay_time);                                // required by SoftwareServo Library to sweep correctly 
                                   // required by SoftwareServo Library to sweep correctly 
  }      
} 
